...
- **`probe(mirror, probe_path, timeout_secs) -> Option<f64>`** — builds a per-URL `ureq::Agent` via `Agent::config_builder().timeout_global()`, fires a GET, returns elapsed seconds or `None` on failure/timeout; ureq 3 reads `*_PROXY` and `NO_PROXY` from environment automatically
